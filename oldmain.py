...
if hasattr(first_part, "function_call"):
    ...
    parts = function_call_result.get("parts", [])
    if not parts:
        raise RuntimeError("Function did not return any parts.")

    function_response = parts[0].get("function_response")
    if not function_response:
        raise RuntimeError("No function_response in first part.")

    result_payload = function_response.get("response")
    if not result_payload:
        raise RuntimeError("Function did not return a valid response.")

    if verbose:
        if "result" in result_payload:
            print(f"-> {result_payload['result']}")
        elif "error" in result_payload:
            print(f"Error: {result_payload['error']}")
        else:
            print("Function call returned no usable result or error.")

    if not result_payload or not hasattr(result_payload, "response"):
        print("Function did not return a valid response.")
